...
TEST(Crc32c, Small) {
  ...
  ASSERT_EQ(4119623852u, common_crc32(0, (unsigned char *)a, strlen(a)));
  ASSERT_EQ(881700046u, common_crc32(1234, (unsigned char *)a, strlen(a)));
  ASSERT_EQ(2360230088u, common_crc32(0, (unsigned char *)b, strlen(b)));
  ASSERT_EQ(3743019208u, common_crc32(5678, (unsigned char *)b, strlen(b)));
}
...
TEST(Crc32c, PartialWord) {
  ...
  memset((void *)a, 1, 5);
  memset((void *)b, 1, 35);
  ASSERT_EQ(2715569182u, common_crc32(0, (unsigned char *)a, 5));
  ASSERT_EQ(440531800u, common_crc32(0, (unsigned char *)b, 35));
  free((void*)a);
  free((void*)b);
}
...
TEST(Crc32c, Big) {
  ...
  memset(a, 1, len);
  ASSERT_EQ(31583199u, common_crc32(0, (unsigned char *)a, len));
  ASSERT_EQ(1400919119u, common_crc32(1234, (unsigned char *)a, len));
  free(a);
}
...
TEST(Crc32c, Performance) {
...

  {
    ...
    unsigned val = common_crc32(0, (unsigned char *)a, len);
  ...
  {
    utime_t start = clock_now();
    unsigned val = common_crc32(0xffffffff, (unsigned char *)a, len);
    utime_t end = clock_now();
    float rate = (float)len / (float)(1024*1024) / (float)(end - start);
  ...
  {
    utime_t start = clock_now();
    unsigned val = crc32_sctp(0, (unsigned char *)a, len);
    utime_t end = clock_now();
    float rate = (float)len / (float)(1024*1024) / (float)(end - start);
  ...
  {
    utime_t start = clock_now();
    unsigned val = crc32_intel_baseline(0, (unsigned char *)a, len);
    utime_t end = clock_now();
    float rate = (float)len / (float)(1024*1024) / (float)(end - start);
  ...
  {
    utime_t start = clock_now();
    unsigned val = crc32_aarch64(0, (unsigned char *)a, len);
    utime_t end = clock_now();
    float rate = (float)len / (float)(1024*1024) / (float)(end - start);
...
TEST(Crc32c, Range) {
  ...
  for (int i = 0 ; i < len; i++, check++) {
    crc = common_crc32(crc, b+i, len-i);
    ASSERT_EQ(crc, *check);
  }
  free(b);
}
...
TEST(Crc32c, RangeNull) {
  ...
  for (int i = 0 ; i < len; i++, check++) {
    crc = common_crc32(crc, NULL, len-i);
    ASSERT_EQ(crc, *check);
  }
}
...
TEST(Crc32c, zeros_performance_compare) {
  ...
  for (size_t scale=1; scale < 31; scale++)
  {
    ...
    pre_start = clock_now();
    start = clock_now();
    uint32_t crc_a = common_crc32(111, nullptr, size);
    end = clock_now();
    ...
    start = clock_now();
#ifdef HAVE_POWER8
    uint32_t crc_b = crc32_zeros(111, size);
#else
    uint32_t crc_b = crc32_func(111, nullptr, size);
#endif
    end = clock_now();
    time_adjusted = (end - start) - (start - pre_start);
#ifdef HAVE_POWER8
    std::cout << "crc32_zeros method. size=" << size << " time=" 
...
TEST(Crc32c, zeros_performance) {
  ...
  for (size_t i=0; i<ITER; i++) {
    for (size_t scale=1; scale < 31; scale++) {
      size_t size = (1<<scale) + rand() % (1<<scale);
      common_crc32(rand(), nullptr, size);
    }
  }
  end = clock_now();
