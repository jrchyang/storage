...
#define BUFFER_ALLOC_UNIT_MAX std::size_t { 256*1024 }
...
static bool buffer_track_crc = get_env_bool("BUFFER_TRACK");
...
public:
  ...
  create(unsigned len, unsigned align,
...
public:
  raw_hack_aligned(unsigned l, unsigned _align) : raw(l) {
    ...
    if (off)
      ...
      data = realdata;
    common_assert(((uintptr_t)data & (align-1)) == 0);
  }
  ~raw_hack_aligned() {
    delete[] realdata;
  }
...
buffer::create_aligned_in_mempool(unsigned len, unsigned align, int mempool) {
  ...
  // size passes 8KB.
  if ((align & ~GLOBAL_PAGE_MASK) == 0 ||
      len >= GLOBAL_PAGE_SIZE * 2) {
#ifndef __CYGWIN__
    return unique_leakable_ptr<buffer::raw>(new raw_posix_aligned(len, align));
#else
    return unique_leakable_ptr<buffer::raw>(new raw_hack_aligned(len, align));
#endif
  }
...
const char *buffer::ptr::raw_c_str() const { common_assert(_raw); return _raw->get_data(); }
unsigned buffer::ptr::raw_length() const { common_assert(_raw); return _raw->get_len(); }
int buffer::ptr::raw_nref() const { common_assert(_raw); return _raw->nref; }
...
unsigned buffer::ptr::append(char c) {
  common_assert(_raw);
  common_assert(1 <= unused_tail_length());
...
unsigned buffer::ptr::append(const char *p, unsigned l) {
  common_assert(_raw);
  common_assert(l <= unused_tail_length());
...
void buffer::ptr::copy_in(unsigned o, unsigned l, const char *src, bool crc_reset) {
  common_assert(_raw);
  common_assert(o <= _len);
  common_assert(o+l <= _len);
...
template<bool is_const>
auto buffer::list::iterator_impl<is_const>::operator +=(unsigned o) -> iterator_impl& {
  p_off +=o;
  while (p != ls->end()) {
    if (p_off >= p->length()) {
      // skip this buffer
...
template<bool is_const>
void buffer::list::iterator_impl<is_const>::copy_shallow(unsigned len, ptr &dest) {
  if (!len) {
    return;
  }
  if (p == ls->end())
...
template<bool is_const>
uint32_t buffer::list::iterator_impl<is_const>::crc32c(size_t length, uint32_t crc) {
  length = std::min<size_t>(length, get_remaining());
  while (length > 0) {
    const char *p;
    size_t l = get_ptr_and_advance(length, &p);
    crc = common_crc32(crc, (unsigned char*)p, l);
    length -= l;
  }
  return crc;
}
...
bool buffer::list::is_aligned_size_and_memory(
    const unsigned align_size, const unsigned align_memory) const {
  for (const auto& node : _buffers) {
...
{
  return is_n_align_sized(GLOBAL_PAGE_SIZE);
}

bool buffer::list::is_page_aligned() const
{
  return is_aligned(GLOBAL_PAGE_SIZE);
}
...
{
  return rebuild_aligned(GLOBAL_PAGE_SIZE);
}
...
void buffer::list::append(char c)
{
  ...
  if (!gap) {
    // make a new buffer!
    auto buf = ptr_node::create(raw_combined::create(BUFFER_APPEND_SIZE, 0, get_mempool()));
...
{
  common_assert(len+off <= bp.length());
  if (!_buffers.empty()) {
    ptr &l = _buffers.back();
...
void buffer::list::substr_of(const list& other, unsigned off, unsigned len)
{
  ...
  common_assert(len == 0 || curbuf != std::cend(other._buffers));
...
void buffer::list::splice(unsigned off, unsigned len, list *claim_by /*, bufferlist& replace_with */)
{
  ...
  while (off > 0) {
    common_assert(curbuf != std::end(_buffers));
...
ssize_t buffer::list::pread_file(const char *fn, uint64_t off, uint64_t len, std::string *error)
{
  ...
  if (fd < 0) {
    int err = errno;
    std::ostringstream oss;
    oss << "can't open " << fn << ": " << common_strerror(err);
    *error = oss.str();
    return -err;
  }
...
int buffer::list::write_file(const char *fn, int mode)
{
  ...
  if (fd < 0) {
    ...
    cerr << "bufferlist::write_file(" << fn << "): failed to open file: "
         << common_strerror(err) << std::endl;
    return -err;
  }
  int ret = write_fd(fd);
  if (ret) {
    cerr << "bufferlist::write_fd(" << fn << "): write_fd error: "
         << common_strerror(ret) << std::endl;
    VOID_TEMP_FAILURE_RETRY(::close(fd));
    return ret;
  }
    ...
    if (node.length()) {
      ...
      if (r->get_crc(ofs, &ccrc)) {
        if (ccrc.first == crc) {
          ...
          */
          crc = ccrc.second ^ common_crc32(ccrc.first ^ crc, NULL, node.length());
          cache_adjusts++;
        }
      } else {
        cache_misses++;
        uint32_t base = crc;
        crc = common_crc32(crc, (unsigned char*)node.c_str(), node.length());
...
bool buffer::ptr_node::dispose_if_hypercombined(buffer::ptr_node* const delete_this)
{
  ...
  if (is_hypercombined) {
    common_assert_always("hypercombining is currently disabled" == nullptr);
...
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_malloc,
                              buffer_raw_malloc,
                              buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_posix_aligned,
                              buffer_raw_posix_aligned,
                              buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_char,
                              buffer_raw_char,
                              buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_claimed_char,
                              buffer_raw_claimed_char,
                              buffer_meta);
MEMPOOL_DEFINE_OBJECT_FACTORY(buffer::raw_static,
                              buffer_raw_static,
                              buffer_meta);
...
void buffer::list::page_aligned_appender::_refill(size_t len) {
  const unsigned alloc =
    std::max(min_alloc,
            ...
            static_cast<unsigned>(GLOBAL_PAGE_SHIFT)));
...
class buffer_error_category : public converting_category {
public:
