...
WRITE_RAW_ENCODER(common_le64)
WRITE_RAW_ENCODER(common_le32)
WRITE_RAW_ENCODER(common_le16)
...
// byte-swapped via the common_le types as above.  The extra conversions
...
# define ENCODE_DUMP_POST(cl)                                                 \
  do {                                                                        \
    ...
    snprintf(fn, sizeof(fn),                                                  \
             ENCODE_STRINGIFY(ENCODE_DUMP_PATH) "/%s__%d.%x",                 \
             #cl, getpid(), i++);                                             \
...
#define WRITE_CLASS_ENCODER(cl)                                               \
  inline void encode(const cl& c, buffer::list &bl, uint64_t features=0) {    \
    ENCODE_DUMP_PRE(); c.encode(bl); ENCODE_DUMP_POST(cl); }                  \
  inline void decode(cl &c, bufferlist::const_iterator &p) { c.decode(p); }
...
inline std::enable_if_t<!traits::supported>
  encode(const std::list<T,Alloc>& ls, bufferlist& bl, uint64_t features)
{
  using counter_encode_t = common_le32;
  unsigned n = 0;
...
#define DECODE_START(v, bl)                                                   \
  ...
  if (v < struct_compat)                                                      \
    throw ::buffer::malformed_input(                                          \
      DECODE_ERR_OLDVERSION(__PRETTY_FUNCTION__, v, struct_compat));          \
  __u32 struct_len;                                                           \
