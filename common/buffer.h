...
unique_leakable_ptr<raw> create_in_mempool(unsigned len, int mempool);
...
unique_leakable_ptr<raw> create_aligned_in_mempool(unsigned len, unsigned align, int mempool);
...
public:
  ...
  bool is_page_aligned() const { return is_aligned(GLOBAL_PAGE_SIZE); }
  bool is_n_align_sized(unsigned align) const {
    return (length() % align) == 0;
  }
  bool is_n_page_sized() const { return is_n_align_sized(GLOBAL_PAGE_SIZE); }
  ...
  void set_offset(unsigned o) {
    common_assert(raw_length() >= o);
    _off = o;
  }
  void set_length(unsigned l) {
    common_assert(raw_length() >= l);
    _len = l;
  }
...
private:
  ...
  static std::unique_ptr<ptr_node, disposer>
  create_hypercombined(unique_leakable_ptr<raw> r);
};

class list {
public:
  // this the very low-level implementation of singly linked list
  // cpputils::buffer::list is built on. We don't use intrusive slist
...
private:
  ...
  public:
    ...
    iterator_impl() : bl(0), ls(0), off(0), p_off(0) {}
...
public:
  ...
  class page_aligned_appender {
    ...
    page_aligned_appender(list *l, unsigned min_pages)
      : bl(*l),
        min_alloc(min_pages * GLOBAL_PAGE_SIZE) {
    }
