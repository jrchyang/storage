...
public:
  ...
  // The second template parameter defaults to the clock's duration type.
  ...
  static void to_common_timespec(const time_point& t, struct common_timespec& ts);
  static struct common_timespec to_common_timespec(const time_point& t);
  static time_point from_common_timespec(const struct common_timespec& ts);
...
inline auto difference(std::chrono::duration<Rep1, Period1> minuend,
  ...
  using srep =
    typename std::common_type<
      ...
      std::chrono::duration<typename std::make_signed<Rep2>::type, Period2>
    >::type;
