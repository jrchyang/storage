...
class error_category : public converting_category {
public:
  error_category(){}
...
bool error_category::equivalent(const boost::system::error_code& c,
                                int ev) const noexcept {
  if (c.category() == boost::system::system_category()) {
    ...
    if (c.value() == boost::system::errc::operation_not_permitted) {
      if (ev == static_cast<int>(errc::conflict)) {
        return true;
      }
    }
  }
