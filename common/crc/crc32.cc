 ...
 */
crc32_func_t choose_crc32(void)
{
  ...
  if (arch_intel_sse42 && crc32_intel_fast_exists()) {
    return crc32_intel_fast;
  }
 ...
 */
crc32_func_t crc32_func = choose_crc32();
...
void create_turbo_table(uint32_t table[32][32])
{
  ...
  for (int bit = 0 ; bit < 32 ; bit++) {
    table[0][bit] = crc32_sctp(1UL << bit, nullptr, 1);
  }
...
uint32_t crc32_zeros(uint32_t crc, unsigned len)
{
  int range = 0;
  ...
  if (remainder > 0)
    crc = common_crc32(crc, nullptr, remainder);
