...
struct denc_traits<
  ...
  std::enable_if_t<
    _denc::is_any_of<_denc::underlying_type_t<T>,
                     common_le64, common_le32, common_le16, uint8_t
...
template<typename T>
inline void denc_varint(T& v, buffer::ptr::const_iterator& p) {
...
denc_lba(uint64_t v, It& p) {
  ...
  v >>= 31 - pos;
  if (!v) {
    *(common_le32*)p.get_pos_add(sizeof(uint32_t)) = word;
    return;
  }
  word |= 0x80000000;
  *(common_le32*)p.get_pos_add(sizeof(uint32_t)) = word;
...
inline std::enable_if_t<is_const_iterator_v<It>>
denc_lba(uint64_t& v, It& p) {
  uint32_t word = *(common_le32*)p.get_pos_add(sizeof(uint32_t));
  int shift = 0;
  switch (word & 7) {
  case 0:
  case 2:
...
struct denc_traits<buffer::list> {
  ...
  static void encode(const buffer::list& v, buffer::list::contiguous_appender& p,
              uint64_t f=0) {
    denc((uint32_t)v.length(), p);
    p.append(v);
  }
  static void decode(buffer::list& v, buffer::ptr::const_iterator& p, uint64_t f=0) {
    uint32_t len;
    denc(len, p);
    v.clear();
...
struct denc_traits<
  ...
  static void encode(const std::pair<A,B>& v, buffer::list::contiguous_appender& p,
                     uint64_t f = 0) {
    if constexpr (featured) {
      denc(v.first, p, f);
      denc(v.second, p, f);
  ...
  buffer::list::const_iterator& p)
{
  ...
  if (!p.is_pointing_same_raw(bl.back()) && remaining > GLOBAL_PAGE_SIZE) {
    traits::decode(o, p);
  } else {
    // ensure we get a contigous buffer... until the end of the
    // buffer::list.  we don't really know how much we'll need here,
...
#define DENC_HELPERS                                                            \
  ...
  _denc_finish(                                                                 \
               ...
               char **len_pos,                                                  \
               uint32_t *start_oob_off) {                                       \
    *(cpputils_le32*)*len_pos = p.get_pos() - *len_pos - sizeof(uint32_t) +     \
...
#define DENC(Type, v, p)                                                        \
  ...
  void encode(                                                                  \
      buffer::list::contiguous_appender& p) const {                             \
    DENC_DUMP_PRE(Type);                                                        \
    _denc_friend(*this, p);                                                     \
  }                                                                             \
  void decode(                                                                  \
...
#define DENC_FEATURED(Type, v, p, f)                                            \
  ...
  void encode(buffer::list::contiguous_appender& p,                             \
              uint64_t f) const {                                               \
    DENC_DUMP_PRE(Type);                                                        \
    _denc_friend(*this, p, f);                                                  \
  }                                                                             \
