...
public:
  ...
  static size_t pick_a_shard_int() {
    ...
    size_t i = (me >> GLOBAL_PAGE_SHIFT) & ((1 << num_shard_bits) - 1);
    return i;
  }
  ...
  void dump(Formatter *f, stats_t *ptotal = 0) const;
};
...
#define MEMPOOL_CLASS_HELPERS()                                               \
  ...
  void *operator new[](size_t size) noexcept {                                \
    common_abort_msg("no array new");                                         \
    return nullptr;                                                           \
  }                                                                           \
  void operator delete(void *);                                               \
  void operator delete[](void *) { common_abort_msg("no array delete"); }
...
#define MEMPOOL_DEFINE_OBJECT_FACTORY(obj, factoryname, pool)                 \
  ...
  void *obj::operator new(size_t size) {                                      \
    return mempool::pool::                                                    \
      alloc_##factoryname.allocate(1);                                        \
  }                                                                           \
  void obj::operator delete(void *p) {                                        \
    return mempool::pool::                                                    \
      alloc_##factoryname.deallocate((obj*)p, 1);                             \
  }
